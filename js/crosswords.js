...
var typingDirection = 'accross'; // 'accross' or 'down'
...
function initGame() {
    ...
    // Listen for translation changes to re-render if needed
    window.addEventListener('langChanged', function() {
        if (document.getElementById('game-screen').style.display === 'block') {
            // If already in game, we strictly shouldn't re-gen on the fly to avoid losing progress
            // but we can update static texts.
        }
    });
}
...
// Procedural generation core logic
function generateCrossword() {
    // 1. Determine size based on difficulty
    ...
    for (var r = 0; r < gridHeight; r++) {
        ...
        for (var c = 0; c < gridWidth; c++) {
            row.push({ isBlack: true, answer: '', user: '', num: 0 }); // Default black
        }
        cellsData.push(row);
    }
    ...
    if (translations && translations[currentLang]) {
        if (targetDifficulty === 'easy' && translations[currentLang].words_short) {
            ...
            // fallback to long or use long for hard
    ...
    for (var i = 0; i < wordBank.length && placedWordsCount < (targetDifficulty==='easy'?3:targetDifficulty==='medium'?5:8) && maxAttempts > 0; i++) {
        ...
        // Ensure word fits the board physically
        if (word.length > gridWidth && word.length > gridHeight) continue;

        if (placedWordsCount === 0) {
            // Place first word roughly in the middle, horizontal
            ...
            // Try to intersect with an existing word
            var placed = false;
                ...
                for (var r = 0; r < gridHeight && !placed; r++) {
                    for (var c = 0; c < gridWidth && !placed; c++) {
                        if (!cellsData[r][c].isBlack && cellsData[r][c].answer === letterToMatch) {
                            // Found intersection. Try placing horizontally and vertically
                            
                            // Try horizontal (meaning matching an existing vertical word)
                            var horizC = c - c1;
                            ...
                            // Try vertical (meaning matching an existing horizontal word)
                            if (!placed) {
                                var vertR = r - c1;
    ...
    // Give numbers and clues to the placed words
...
function canPlaceWord(word, startR, startC, dC, dR) {
    ...
    // Check if bounds interfere with other words improperly
    for (var i = 0; i < word.length; i++) {
        var r = startR + (i * dR);
        var c = startC + (i * dC);
        var cell = cellsData[r][c];
        
        // If cell is occupied by a different letter
        if (!cell.isBlack && cell.answer !== word.charAt(i)) {
            return false;
        }
        
        // If cell is blank but adjacent cells are occupied (no parallel touching words)
        if (cell.isBlack) {
            if (dC === 1) { // Placing horizontally
    ...
    // Check before and after word bounds to ensure we don't merge words into one big word
    if (dC === 1) {
        ...
        for (var c = 0; c < gridWidth; c++) {
            ...
            if (startsAcross || startsDown) {
                ...
                if (startsAcross) {
                    ...
                    var theWord = "";
                    for (var k = 0; k < usedWordsList.length; k++) {
                        if (usedWordsList[k].dC === 1 && usedWordsList[k].r === r && usedWordsList[k].c === c) {
                            theWord = usedWordsList[k].word;
                            break;
                        }
                    }
                    if (!theWord) {
                ...
                if (startsDown) {
                    var theWordD = "";
                    for (var m = 0; m < usedWordsList.length; m++) {
                        if (usedWordsList[m].dR === 1 && usedWordsList[m].r === r && usedWordsList[m].c === c) {
                            theWordD = usedWordsList[m].word;
                            break;
                        }
                    }
                    if (!theWordD) {
...
// Generates an anagram to use as a clue since we don't have descriptions
function scrambleWord(word) {
    ...
    while(attempts > 0) {
        ...
        var res = arr.join('');
        if (res !== word) {
            return res.split('').join(' '); // Add spaces for readability on e-ink
        }
        attempts--;
    }
    return arr.join(' ');
...
function renderBoard() {
    ...
    // Scale board width dynamically based on cell size
    var boardPixelWidth = (gridWidth * 32); // 30px width + 2px margin/border
    ...
    for (var r = 0; r < gridHeight; r++) {
        for (var c = 0; c < gridWidth; c++) {
            ...
            if (cell.isBlack) {
                // Kindle Flash mitigation: empty space needs rigid body "0" hidden
                ...
                var isRowSelected = (selectedCell && typingDirection === 'accross' && selectedCell.r === r && isWordBlock(r, selectedCell.c, r, c)) ? ' typing-row' : '';
                ...
                var letterHtml = cell.user !== '' ? cell.user : '<span style="visibility:hidden">0</span>';
                
                boardHtml += '<div class="cw-cell' + highlightSelected + '" onclick="selectCell(' + r + ',' + c + ')">';
                boardHtml += numHtml;
...
// Checks if the target cell is part of the same contiguous word block as the source cell
function isWordBlock(startR, startC, targetR, targetC) {
    if (typingDirection === 'accross') {
...
function renderKeyboard() {
    ...
    for (var i = 0; i < rows.length; i++) {
        ...
        if (i === 2) {
            kbHtml += '<button class="btn cw-key" style="width: 50px;" onclick="typeLetter(\'DEL\')">DEL</button>';
        }
        kbHtml += '</div><br/>';
    }
...
function selectCell(r, c) {
    ...
    if (selectedCell && selectedCell.r === r && selectedCell.c === c) {
        // Toggle direction
        typingDirection = (typingDirection === 'accross') ? 'down' : 'accross';
    } else {
...
function typeLetter(letter) {
    ...
    if (letter === 'DEL') {
        ...
        // Auto-move backwards if possible
        if (typingDirection === 'accross') {
            if (c - 1 >= 0 && !cellsData[r][c-1].isBlack) {
                selectedCell.c = c - 1;
            }
        } else {
            if (r - 1 >= 0 && !cellsData[r-1][c].isBlack) {
                selectedCell.r = r - 1;
            }
        }
    ...
    // Auto-move forwards if possible
    if (typingDirection === 'accross') {
        if (c + 1 < gridWidth && !cellsData[r][c+1].isBlack) {
            selectedCell.c = c + 1;
        }
    } else {
        if (r + 1 < gridHeight && !cellsData[r+1][c].isBlack) {
            selectedCell.r = r + 1;
        }
    }
...
function checkWinCondition() {
    ...
    if (isFull) {
        if (isCorrect) {
            ...
            // Disable keyboard and selection
            selectedCell = null;
            document.getElementById('crosswords-kb').innerHTML = '';
            renderBoard();
        } else {
            // Optional: visual indication that it's wrong? 
            // Kindle e-ink discourages heavy animations, keep it simple.
        }
    } else {
        document.getElementById('status-message').innerHTML = '';
    }
